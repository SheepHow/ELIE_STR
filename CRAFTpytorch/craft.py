...
class CRAFT_feat_align(nn.Module):
    ...
    def forward(self, x):
        ...
        y2 = F.interpolate(y1, size=sources[2].size()[2:], mode='bilinear', align_corners=False)
        y2 = torch.cat([y2, sources[2]], dim=1)
        y3 = self.upconv2(y2)

        y4 = F.interpolate(y3, size=sources[3].size()[2:], mode='bilinear', align_corners=False)
        y4 = torch.cat([y4, sources[3]], dim=1)
        y5 = self.upconv3(y4)

        y6 = F.interpolate(y5, size=sources[4].size()[2:], mode='bilinear', align_corners=False)
